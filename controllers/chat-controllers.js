...
const getAllCurrentChats = (req, res, next) => {
    const sql = `SELECT * FROM chats WHERE sender = '${req.params.id}' OR receiver = '${req.params.id}' ORDER BY updatedAt ASC`
...
const getChatMessages = (req, res, next) => {
    const sql = `SELECT * FROM chat_messages WHERE chatID = '${req.params.id}' ORDER BY timeSent ASC`
    connection.query(sql, (err, response) => {
        if (err) return res.status(422).json({message : err.sqlMessage}) 
